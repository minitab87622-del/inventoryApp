...
class Database:
    def __init__(self, path=None):
        if path is None:
            try:
                ...
                data_dir = app.user_data_dir if (app and app.user_data_dir) else ""
            except:
                data_dir = ""
    ...
    def get_items(self):
        cur = self.conn.cursor()
        cur.execute("SELECT name, quantity, imported, sold FROM items ORDER BY id ASC")
        return cur.fetchall()
    ...
    def add_import(self, name, amount):
        ...
        if not name or amount <= 0:
            return False
        cur = self.conn.cursor()
        cur.execute("UPDATE items SET quantity = quantity + ?, imported = imported + ? WHERE name=?", (amount, amount, name))
        if cur.rowcount == 0:
            return False
        cur.execute("INSERT INTO movements(item, action, amount) VALUES(?,?,?)", (name, "وارد", amount))
        self.conn.commit()
        return True
    ...
    def add_sale(self, name, amount):
        ...
        if current_qty < amount:
            return False
        cur.execute("UPDATE items SET quantity = quantity - ?, sold = sold + ? WHERE name=?", (amount, amount, name))
        cur.execute("INSERT INTO movements(item, action, amount) VALUES(?,?,?)", (name, "بيع", amount))
        self.conn.commit()
        return True
